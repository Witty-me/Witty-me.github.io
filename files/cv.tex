...
\address{Building N9, 101 Software Avenue, Nanjing, Jiangsu, 210012}
...
\begin{cventries}
	...
	{Huawei} % Institution
	{Nanjing/Shanghai, China} % Location
	{2025.08 - Present} % Date(s)
	{
		\begin{cvitems} % Description(s) bullet points
			\item {Working in the WLAN Lab (Data Communication Product Line), focusing on AI technologies and their applications to indoor wireless networking and sensing (Rated "Excellent" upon full-time conversion).}
			\item {Spearheaded the design and development of end-to-end AI algorithm architectures, leading the pre-research and deployment of intelligent sensing models (leveraging signals like Wi-Fi CSI) for smart security and building scenarios.}
			\item {Built a comprehensive closed-loop data pipeline (collection, cleaning, and augmentation), and designed and trained neural networks based on CNN and Attention mechanisms for sensing task models.}
			\item {Addressed data imbalance and cross-environment generalization challenges by designing an autoencoder-based anomaly detection model utilizing only negative samples; introduced the ADDA algorithm to achieve cross-domain transfer from the lab to real settings.}
			\item {Executed lightweight deployment and performance evaluation of algorithms on edge devices, and conducted real-world testing in real environments to ensure the product integration of neural network models.}
	\end{cvitems}
}
