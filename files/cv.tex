...
\begin{cventries}
	...
	%---------------------------------------------------------
	\cventry
	{Senior Engineer} % Degree
	{Huawei} % Institution
	{Shanghai, China} % Location
	{2025.08 - Present} % Date(s)
	{
		\begin{cvitems} % Description(s) bullet points
			\item {Working in the WLAN Lab (Data Communication Product Line), focusing on AI for CSI-based sensing and physical-world understanding (received an ``Excellent'' rating upon completion of probation).}
			\item {Spearheaded the design and development of end-to-end AI algorithm architectures, leading the pre-research and deployment of intelligent sensing models for smart security and smart building scenarios.}
			\item {Developed lightweight CNN-based sensing models for practical deployments, incorporating anomaly detection, data augmentation, and ADDA-based domain adaptation to address limited labels, data imbalance, and environmental variations.}
			\item {Designed a self-supervised Wi-Fi CSI pretraining framework with domain-specific tokenization and physics-guided objectives, enabling transferable representations for fine-tuning across sensing environments and downstream tasks.}
			\item {Worked cross-functionally to build closed-loop data pipelines spanning collection, cleaning, augmentation, and evaluation, while optimizing and deploying sensing models on edge devices for real-world validation and product integration.}
	\end{cvitems}
}
