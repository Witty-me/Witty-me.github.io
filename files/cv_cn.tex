...
\begin{cventries}
	...
	{2025.08 - 至今} % Date(s)
	{
		\begin{cvitems} % Description(s) bullet points
			\item {就职于华为数据通信产品线— WLAN 技术实验室，聚焦面向 CSI 感知与物理世界理解的 AI 技术（转正考核获评“优秀”）。}
			\item {主导端到端 AI 算法架构的设计与开发，负责面向智慧安防与智慧楼宇等场景的智能感知模型预研与产品化落地。}
			\item {面向实际部署开发轻量化 CNN 感知模型，结合异常检测、数据增强与基于 ADDA 的领域自适应技术，缓解标签有限、数据不平衡及环境差异等问题。}
			\item {设计面向 Wi-Fi CSI 的自监督预训练框架，通过领域专属的 Tokenization 方法与物理原理驱动的训练目标，学习可迁移表征，并支持模型在不同感知环境和下游任务上的微调。}
			\item {协作构建覆盖数据采集、清洗、增强与评估的闭环数据流水线，完成感知模型在边缘设备上的优化与部署，支持真实场景测试与产品落地。}
		\end{cvitems}
	}
