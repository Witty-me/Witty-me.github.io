...
\begin{cventries}
	...
	%---------------------------------------------------------
	\cventry
	{高级工程师(数据通信产品线—WLAN技术实验室)} % Degree (这里指职位)
	{华为} % Institution
	{中国 上海} % Location
	{2025.08 - 至今} % Date(s)
	{
		\begin{cvitems} % Description(s) bullet points
			\item {主导面向物理世界感知的 Wi-Fi CSI 算法架构设计与研发，覆盖智能感知模型从预研、开发到部署的完整流程，并应用于智慧安防、智慧园区等场景。}
			\item {面向端侧部署，开发轻量化 CNN 感知模型，通过训练阶段的数据增强与基于 ADDA 的领域自适应技术，缓解数据不平衡问题，提升模型应对环境域偏移的鲁棒性。}
			\item {设计面向 Wi-Fi CSI 的 Transformer Encoder 自监督表征学习框架，引入 CSI 专用的 Token 构建方法与物理机理引导的预训练目标，学习可在不同感知环境和下游任务间迁移的通用表征。}
			\item {通过跨团队协作，改进信号处理方法并将其融入 AI 感知流程，完成面向端侧的模型优化与部署，推动真实场景验证与产品集成。}
			\item {试用期考核获评“优秀”。}
		\end{cvitems}
	}
	
	\cventry
	{研究实习生（上海无线组；导师：邱锂力教授）} % Degree
	{微软亚洲研究院} % Institution
	{中国 上海} % Location
...
\begin{cventries}
	...
	{
		\begin{cvitems} % Description(s) bullet points
			\item {一套激光自对准系统，支持高速移动场景下的自由空间光通信。}
		\end{cvitems}
	}
